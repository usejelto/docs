---
title: "Install on HTML"
group: install
slug: install/html
summary: "Add Jelto to your HTML website and verify the published pages."
---

# Install on HTML

Measure visits to your HTML website using the script from your Jelto product.
...
1. Edit the shared HTML template used by all pages. Put the copied tag immediately before `</head>`. If pages do not share a template, add it to each HTML document.
2. Preserve the copied `data-product`, `data-endpoint` and `src` values.
3. Upload or deploy the changed HTML files to your normal website host.
...
Do not put the script in an iframe or display it as a code sample.
...
Platform reference: [Official HTML documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script).
